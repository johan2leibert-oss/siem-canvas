...
const EventsTab = () => {
  const [allEvents] = useState<EventRecord[]>(() => generateEvents(200));
  ...
  const filtered = useMemo(() => {
    ...
    return data;
  }, [allEvents, eventTypeFilter, eventNameFilter, severityFilter, sourceFilter, searchIp, dateFrom, dateTo, sortDir]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const pageData = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  const uniqueNames = ["All", "NA", ...Array.from(new Set(allEvents.map((e) => e.eventName)))];
